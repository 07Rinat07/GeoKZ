...
Функцию нельзя считать завершённой, если пользовательская инструкция или актуальный план отсутствуют хотя бы на одном из трёх языков. В документации необходимо явно различать: реализовано, в разработке, запланировано.
...
Пайдаланушы нұсқаулығы немесе өзекті жоспар үш тілдің кемінде бірінде жоқ болса, функция аяқталды деп саналмайды.
...
A user-facing feature is not complete if either the user instructions or the current roadmap are missing in any of the three supported languages. Documentation must clearly distinguish implemented, in-development and planned functionality.
